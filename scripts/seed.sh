...
event=$(curl -sS -X POST "$BASE_URL/admin/events" \
...
tt=$(curl -sS -X POST "$BASE_URL/admin/events/$event_id/ticket_types" \
  -H 'content-type: application/json' \
...
echo "Done. Try seckill:"
echo "  curl -X POST $BASE_URL/seckill -H 'content-type: application/json' -H 'idempotency-key: demo1' -d '{\"user_id\":\"u1\",\"ticket_type_id\":\"'$tt_id'\",\"qty\":1}'"
